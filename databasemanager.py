...
def set_guild_mute_role(guild_id: int, role_id: int):
    cur.execute(
        "UPDATE guilds SET mute_role_id = ? WHERE guild_id = ?", (role_id, guild_id)
    )
    conn.commit()
...
#Muted users table
def create_mute(user_id: int, duration: int, guild_id: int, reason: str):
    query_user = check_db_for_user(user_id)
    ...
    cur.execute(
        f"INSERT INTO muted_users (user_id, mute_expiration, guild_id, reason) VALUES (?, ?, ?, ?)", (user_id, duration, guild_id, reason)
    )
    conn.commit()

#Check for muted users
def check_mutes(current_time: int):
    cur.execute(
